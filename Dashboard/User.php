...
<section class="accounts">
      ...
      if($select_account->rowCount() > 0){
         while($fetch_accounts = $select_account->fetch(PDO::FETCH_ASSOC)){ 
            ...
            if(isset($_POST['updatetk']) && isset($_POST['user_id']) && $_POST['user_id'] == $user_id){
               $update_role = $conn->prepare("UPDATE `users` SET role = ' staff' WHERE id = ?");
      ...
      <div class="flex-btn">
            ...
            <input type="hidden" name="user_id" value="<?= $user_id ?>">
               <div class="button-container">
                    <button type="submit" name="updatetk" class="updatetk-btn">Cấp quyền QL</button>
   ...
   <?php
      ...
      echo '<p class="empty">Chưa có tài khoản khách hàng nào</p>';
   }
   ?>
   </div>
</section>
