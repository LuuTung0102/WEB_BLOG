...
<section class="show-posts">
   <h1 class="heading">Bài chưa được hoạt động</h1>
   <div class="box-container">
      <?php
        $select_posts = $conn->prepare("SELECT post.*, users.username, users.avatar, category.catname FROM `post` INNER JOIN `users` ON post.author_id = users.id INNER JOIN `category` ON post.cat_id = category.cat_id WHERE post.status = 'deactive' AND post.author_id = ? ORDER BY post.date DESC");
...
<section class="show-posts">
   ...
   <div class="box-container">
      <?php
         $select_posts = $conn->prepare("SELECT post.*, users.username, users.avatar, category.catname FROM `post` INNER JOIN `users` ON post.author_id = users.id INNER JOIN `category` ON post.cat_id = category.cat_id WHERE post.status = 'deactive' ORDER BY post.date DESC");
